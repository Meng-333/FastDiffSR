    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for _, train_data in enumerate(train_loader):
                ...
                #lr rate --- change
                # if current_step % 100000 == 0:    # 每100k减半
                    # diffusion.optG.param_groups[0]['lr'] = diffusion.optG.param_groups[0]['lr'] / 2
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    if current_step == 9152:
                        logger_val.info(
                            '<epoch:{:3d}, iter:{:8,d}, lr:{:.7e}> bic_mse: {:.5e}, bic_psnr: {:.5e}, bic_ssim：{:.5e}, bic_ergas: {:.5e}, bic_lpips: {:.5e}'.format(
                                current_epoch, current_step, diffusion.optG.param_groups[0]['lr'], bic_mse, bic_psnr,
                                bic_ssim, bic_ergas, bic_lpips))
                    logger_val.info(
                        '<epoch:{:3d}, iter:{:8,d}, lr:{:.7e}> sr_mse: {:.5e}, sr_psnr: {:.5e}, sr_ssim：{:.5e}, sr_ergas: {:.5e}, sr_lpips：{:.5e}'.format(
                            current_epoch, current_step, diffusion.optG.param_groups[0]['lr'], avg_mse, avg_psnr, avg_ssim, avg_ergas, avg_lpips))
                    # tensorboard logger
        ...
        logger_val.info('<epoch:{:3d}, iter:{:8,d}> sr_mse: {:.5e}, sr_psnr: {:.5e}, sr_ssim：{:.5e}, sr_ergas: {:.5e}, sr_lpips：{:.5e}'.format(
