...
class GeneratorResNet(nn.Module):
    def __init__(self,
                 ...
                 # window_size=8,  # when scale =3,9    216*216
                 # window_size=9,  #when scale =2,4,8   216*216
                 #window_size=8,    # when scale =4    256*256
                 window_size=16,    # when scale =8    256*256
                 compress_ratio=3,
                 squeeze_factor=30,
                 conv_scale=0.01,
                 overlap_ratio=0.5,
...
class HAT(object):
    ...
    def train(self):
        ...
        if self.epoch != 0:
            ...
            #G_model_path = '/g12238036mfe/mfe/ESRRDAN/Result/model/hat_x3_generator_param.pkl'
            ...
            for i, (input, target, bc_input, imgpath) in enumerate(test_data_loader):
                ...
                for i in range(self.test_batch_size):
                    ...
                    if save_img:
                        ...
                        ergas = [None, None, bc_ergas, recon_ergas]
                        lpips = [None, None, bc_lpips, recon_lpips]
                        indicators = {"MSE": mses, "PSNR": psnrs, "SSIM": ssims, "ERGAS": ergas, "LPIPS": lpips}
    ...
    def dm_test_single(self, img_fn, modelpath=None):
        # networks
        cuda = True if torch.cuda.is_available() else False
        self.generator = GeneratorResNet(upscale=4, in_chans=3, img_size=128)
        ...
        for img_ in imgs:
            ...
            srutils.save_img1(recon_imgs[0].cpu().data, self.save_dir, self.save_dir + '/SR_hat_x4_%s' % img_)
